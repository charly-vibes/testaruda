...
- [ ] 1.1 Add a `shell-words` crate dependency; implement a `spawn_adapter(command_string, timeout)` helper in `src/main.rs` (or `src/adapter.rs`) that shell-splits the configured string into `(binary, args)` and calls `AdapterIO::spawn`. Route all five existing `AdapterIO::spawn` call sites (`src/main.rs` lines 448, 579, 803, 858, 904) through this helper so the split is applied by construction — not just at two sites.
- [ ] 1.2 Unit test: `"testaruda-adapter-rust"` → `("testaruda-adapter-rust", [])` (no regression for bare-binary adapters)
- [ ] 1.3 Unit test: `"titi testaruda-adapter"` → `("titi", ["testaruda-adapter"])`
- [ ] 1.4 Unit test: `"julia --project=. -e 'using Testimonial; Testimonial.Protocol.run_adapter_protocol()'"` → `("julia", ["--project=.", "-e", "using Testimonial; Testimonial.Protocol.run_adapter_protocol()"])` (quotes handled correctly, `-e` body preserved as one arg)
- [ ] 1.5 Unit test: empty command string `""` → config-error diagnostic naming the extension; do NOT call `AdapterIO::spawn` with empty binary (per TIA-ADAPT-024 "Empty command string" scenario)
- [ ] 1.6 Unit test: quoted binary path with spaces `"\"C:\\Program Files\\titi\\titi.exe\" testaruda-adapter"` → `("C:\Program Files\titi\titi.exe", ["testaruda-adapter"])` (per TIA-ADAPT-024 "Binary path with spaces" scenario)
...
- [ ] 2.1 Document the `testaruda.toml` extension mappings for .NET in the adapter's user-facing docs (including `.cs`, `.fs`, `.vb` source extensions):
  ```toml
  [adapters.extensions]
...
- [ ] 3.1 When `titi` is not on `PATH` and the user has configured the titi mapping, `testaruda select` SHALL fall back to full-suite selection per TIA-ADAPT-012 and record the missing-binary failure (verify the existing `AdapterIO::spawn` NotFound path already handles this — no new code expected once shell-split is in place). The failure diagnostic SHALL name the resolved binary (`titi`), not the full command string.
- [ ] 3.2 When the titi adapter exits non-zero (e.g. graph-build failure during handshake), the core SHALL fall back to full-suite per TIA-ADAPT-012 (verify existing `read_response`/`ProcessExit` path handles this)
...
## 5. Documentation
- [ ] 5.1 Update `docs/getting-started.md` or the adapter list to include the .NET / titi adapter, noting it is an external binary (not a workspace crate), is opt-in (not auto-detected, following the Julia precedent), and linking to `github.com/sashakile/titi` for installation
- [ ] 5.2 Note in `testaruda.toml` example comments that extension-mapping values are command strings that the core shell-splits (per TIA-ADAPT-024), so multi-token invocations (e.g. `titi testaruda-adapter`, `julia --project=. -e '...'`) are supported
...
## 7. Design document
- [ ] 7.1 Write `design.md` covering: the `shell-words` crate decision (over `shlex`), the five-spawn-site refactor via `spawn_adapter` helper, the `.cs`/`.fs`/`.vb` routing rationale, the opt-in-vs-detection decision (following Julia), and the polyglot-routing acknowledgment (misrouted files → titi `unresolved` → TIA-SAFE-004)
